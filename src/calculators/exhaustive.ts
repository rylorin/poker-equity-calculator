...
export class ExhaustiveCalculator extends EquityCalculator {
  ...
  async calculate(): Promise<EquityResult> {
    ...
    const shouldUseExhaustive = 
      this.options.forceExhaustive || 
      totalCombinations <= (this.options.maxExhaustiveCombinations || 25000);
    ...
    const incompleteHands = this.hands.map((hand, index) => ({
      handIndex: index,
    ...
    if (incompleteHands.length === 0 && boardCardsNeeded === 0) {
      ...
      const boardCompletions: Card[][] = 
        boardCardsNeeded > 0 
          ? generateCombinations(remainingCards, boardCardsNeeded) 
          : [[]];
    ...
    for (const result of playerResults) {
      result.equity = (result.wins + result.ties / 2) / totalHands;
    }
